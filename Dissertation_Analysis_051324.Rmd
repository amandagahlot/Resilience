...
```{r subset severity openness, echo=FALSE, include=TRUE}
#significant difference in ACSS between groups. t-test for t value
df_mod <- subset(df, severity == 2)
df_severe <- subset(df, severity == 3)
...
```{r model included acsi, echo=FALSE, include=TRUE}
summary(model6)
```
...
```{r mediation acss apathy dep, echo=FALSE, include=TRUE}
# Initial Model
...
plot(mediation)
```
```{r path mediation dep, echo=FALSE, include=TRUE}
...
```
```{r mediation apathy fatigue, echo=FALSE, include=TRUE}
# Initial Model
...
plot(mediation)
```
```{r path mediation fatigue, echo=FALSE, include=TRUE}
...
```{r mediation apathy gencog, echo=FALSE, include=TRUE}
# Initial Model
...
```{r path mediation gencog, echo=FALSE, include=TRUE}
...
```{r vis global 3 to save, results='asis', echo=FALSE, include=TRUE}
library("ggpubr")
...
```{r timeinjury predictive variables, fig.width=15,fig.height=15, echo=FALSE, include=TRUE}
library(ggplot2)
library(reshape2)
library(Hmisc)

# Define QOL_PPF_variables
Time_PPF_variables <- c("TimeSinceInjury", "Global", "IADL", "Leisure", "Fitness", "Social", "PhysicalHealth", "GeneralCognition", "EmotionalHealth", "Extraversion", "Agreeable", "Consciousness", "Neuroticism", "Openness", "income", "MaritalStatus", "SocialSupport", "Apathy", "Disinhibition", "ExecFunc", "Total")

# Ensure selected variables are in the dataframe
Time_PPF_variables <- intersect(Time_PPF_variables, colnames(df2))

# Extract relevant data
Time_PPF_df2 <- df2[, Time_PPF_variables]

# Calculate correlation matrix
cor_matrix <- rcorr(as.matrix(Time_PPF_df2), type = "spearman")$r
cor_matrix[upper.tri(cor_matrix)] <- NA
p_matrix <- rcorr(as.matrix(Time_PPF_df2), type = "spearman")$P
p_matrix[is.na(p_matrix)] <- .0000001
p_matrix[upper.tri(p_matrix)] <- NA
