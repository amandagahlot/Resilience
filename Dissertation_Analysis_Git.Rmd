---
title: "The Role of Self-Regulation on Resiliency after TBI"
...
setwd("/Users/amandagahlot/Documents/Dissertation/GitDiss")
```
...
```{r acs compare,  results='asis', include=TRUE}
acs_table <- df_demo %>%
...
```{r subset severity, include=TRUE}
...
```{r frsbe compare,  results='asis', include=TRUE}
frsbe_table <- df_demo %>%
...
```{r tbiqol compare, results='asis',  include=TRUE}
...
```{r included variables compare, results='asis', include=TRUE}
PPF_table <- df_demo %>%
...
```{r subset severity openness, include=TRUE}
#significant difference in ACSS between groups. t-test for t value
df_mod <- subset(df, severity == 2)
df_severe <- subset(df, severity == 3)
...
```{r descriptive, include=TRUE, results='asis'}
...
```{r table ACS mean(sd), results='asis', include=TRUE}
mean_sd_acs <- df_demo %>%
...
```{r all variables, fig.width=15,fig.height=15, results='asis', include=TRUE}
library(ggplot2)
library(reshape2)
library(Hmisc)
...
```{r ppf_only corr, fig.width=15,fig.height=15, include=TRUE}
library(ggplot2)
library(reshape2)
library(Hmisc)
...
```{r subscale corr, fig.width=15,fig.height=15, include=TRUE}
...
```{r qol and frsbe corr, fig.width=10,fig.height=8, include=TRUE}
...
```{r summary models AIC acss, include=TRUE}
library(AICcmodavg)
#composite scores
...
```{r summary model selected acss, include=TRUE}
summary(model7)
```
### ACS3 IADL

```{r summary of models AIC acgi, include=TRUE}
library(AICcmodavg)
#composite scores
...
```{r model included acsi, include=TRUE}
summary(model6)
```
...
```{r leisure acs AIC,include=TRUE}
...
```{r summary selected model acsl, include=TRUE}
summary(model6)
```

### ACS3 Fitness

```{r fitness AIC, include=TRUE}
...
```{r load library mediation, include=FALSE}
library(mediation)
library(JSmediation)
```
#### Physical Health
...
```{r path mediation phys,  include=TRUE}
library(diagram)
...
```{r cog health index, include=TRUE}
# MEDIATION
# Initial Model
...
```{r mediation emo health,  include=TRUE}
# Initial Model
...
```{r path mediation emo, include=TRUE}
...
```{r mediation social support, include=TRUE}
# Initial Model
...
```{r path mediation social, include=TRUE}
...
```{r mediation acss apathy dep, include=TRUE}
# Initial Model
...
```{r path mediation dep, include=TRUE}
...
```{r mediation apathy fatigue, include=TRUE}
# Initial Model
...
```{r path mediation fatigue, include=TRUE}
...
```{r mediation apathy gencog, include=TRUE}
# Initial Model
...
```{r path mediation gencog, include=TRUE}
...
a.	Is time after injury associated with an individual’s ability to re-engage in meaningful activities?
...
```{r summary time since injury, include=TRUE}
...
print(summary_table)
```
```{r mean sd of time since injury, echo=FALSE, include=TRUE}
mean_tsi <- mean(df$time_injury, na.rm = TRUE)
std_tsi <- sd(df$time_injury, na.rm = TRUE)
print(mean_tsi)
print(std_tsi)
...
```{r corr time since injury with acsg, fig.width=15,fig.height=15, include=TRUE}
# Define variables
...
```{r regression time since injury, include=TRUE}
...
```{r regression time since injury social,include=TRUE}
...
```{r regression time since injury IADL, include=TRUE}
...
```{r regression time since injury leisure, include=TRUE}
...
```{r regression time since injury fitness, include=TRUE}
...
```{r creating new groups, include=FALSE}
df_time3 <- read_excel('cleaned_data_Final.xlsx')
...
```{r demo 3 groups, results='asis', include=TRUE}
...
```{r counts of new groups, include=TRUE}
...
```{r vis global 3 to save, results='asis', include=TRUE}
library("ggpubr")
...
```{r vis global 3 again, include=TRUE}
library("ggpubr")
...
```{r anova global extreme, include=TRUE}
res.aov <- aov(acsg_retain~time_injury_ex, data=df_time3)
summary(res.aov)
```

```{r lm global,  include=TRUE}
...
```{r vis Social 3 again, include=TRUE}
library("ggpubr")
...
```{r anova social 3, include=TRUE}
res.aov <- aov(acss_retain~time_injury_ex, data=df_time3)
summary(res.aov)
```

```{r lm social 3, include=TRUE}
...
```{r vis IADL 3 again, include=TRUE}
library("ggpubr")
...
```{r vis Leisure 3 again, include=TRUE}
library("ggpubr")
...
```{r anova Leisure 3, include=TRUE}
res.aov <- aov(acsl_retain~time_injury_ex, data=df_time3)
summary(res.aov)
```

```{r lm Leisure 3, include=TRUE}
...
```{r  visualize Fitness 3, include=TRUE}
library("ggpubr")
ggboxplot(df_time3, x = "time_injury_ex", y = "acsg_retain",
          color = "time_injury_ex", palette = c("#00AFBB", "#E7B800", "#FC4E07"),
          order = c("Early", "Mid", "Later"),
...
```{r anova Fitness 3, include=TRUE}
res.aov <- aov(acsf_retain~time_injury_ex, data=df_time3)
summary(res.aov)
```

```{r lm Fitness 3, include=TRUE}
